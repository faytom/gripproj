...
static int epb_get_varint32_bits(const uint8 *data, int len, uint32 *value);

inline static int sizeof_tag(uint16 tag)
{
...
static int epb_get_value_length(const uint8 *data, int len, uint32 wire_type)
{
    int offset = 0;
    switch(wire_type) {
...
static int epb_get_tag_value_offset(const uint8 *data, int len, uint16 tag)
{
    int offset = 0;
    uint32 p_tag;
    while (offset < len) {
...
static uint32 epb_get_fixed32_bits(const uint8 *data, int len)
{
    if(len < 4)
        return 0;
...
{
    uint32 bits = epb_get_fixed32(e, tag);
    return *(float *)&bits;
}
...
static int epb_pack_tag(Epb *e, uint16 tag)
{
...
static int epb_pack_varint32_bits(Epb *e, uint32 value)
{
    uint8 buf[5] = {0};
    int i = 0;
    do {
...
static int epb_set_fixed32_bits(Epb *e, uint32 value)
{
...
static int epb_pack_length_delimited(Epb *e, const uint8 *data, int len)
{
...
int epb_set_bytes(Epb *e, uint16 tag, const uint8 *data, int length)
{
    int len = 0;
    int ret = epb_pack_tag(e, tag);
    if (ret < 0) return ret;
