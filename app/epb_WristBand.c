...
static int epb_mmopen_unpack_base_response(MMOpen_BaseResponse *base_response, const uint8 *buf, int buf_len)
{
    Epb epb;
    ...
    if (epb_has_tag(&epb, TAG_MMOpen_BaseResponse_ErrMsg)) {
        base_response->err_msg.str = epb_get_string(&epb, TAG_MMOpen_BaseResponse_ErrMsg, &base_response->err_msg.len);
...
int epb_mmopen_pack_step_data_item(MMOpen_StepDataItem *item, uint8 *buf, int buf_len)
{
	Epb epb;
...
int epb_mmopen_wristband_request_pack_size(MMOpen_WristbandRequest *request)
{
	...
	if (request->has_ext_data) {
		pack_size += epb_length_delimited_pack_size(TAG_MMOpen_WristbandRequest_ExtData, request->ext_data.len);
	}
...
int epb_mmopen_pack_wristband_request(MMOpen_WristbandRequest *request, uint8 *buf, int buf_len)
{
	...
	for (int i=0; i<request->step_data_count; i++) {
		...
		uint8 *item_buf = (uint8 *)osal_mem_alloc(item_len);
		ret = epb_mmopen_pack_step_data_item(item, item_buf, item_len);
		if (ret < 0) {
			osal_mem_free((void *)item_buf);
			return ret;
		}
	...
	if (request->has_ext_data) {
		ret = epb_set_bytes(&epb, TAG_MMOpen_WristbandRequest_ExtData, request->ext_data.data, request->ext_data.len);
		if (ret < 0) return ret;
	}
...
MMOpen_WristbandResponse *epb_mmopen_unpack_wristband_response(const uint8 *buf, int buf_len)
{
    ...
    MMOpen_WristbandResponse *response = (MMOpen_WristbandResponse *)osal_mem_alloc(sizeof(MMOpen_WristbandResponse));
    osal_memset(response, 0, sizeof(MMOpen_WristbandResponse));
    int base_response_len = 0;
    const Message *base_response = epb_get_message(&epb, TAG_MMOpen_WristbandResponse_BaseResponse, &base_response_len);
    int ret = epb_mmopen_unpack_base_response(&response->base_response, base_response, base_response_len);
    if (ret < 0) {
        osal_mem_free((void *)response);
        return NULL;
    }
